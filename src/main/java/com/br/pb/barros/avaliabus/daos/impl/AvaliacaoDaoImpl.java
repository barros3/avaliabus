...
public class AvaliacaoDaoImpl implements AvaliacaoDao {
	...
	public List<Avaliacao> all() {
		return manager.createQuery("select p from Avaliacao p", Avaliacao.class).getResultList();
	}
	...
	public PaginatedList paginated(int page, int max) {
		return new PaginatorQueryHelper().list(manager, Avaliacao.class, page, max);
	}
	...
	public Map<String, Integer> avalieChartConservacaoColetivo(Empresa empresa) throws SQLDataException, JsonProcessingException {
		...
		while (conservacaoColetivo <= 5) {
			...
			Query conservacaoColetivoTemp = manager
					...
					.setParameter("empresa", empresa.getId()).setParameter("conservacaoTransporte", conservacaoColetivo);
